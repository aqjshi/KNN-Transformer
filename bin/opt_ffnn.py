...
device = torch.device("mps") if torch.backends.mps.is_available() else torch.device("cuda" if torch.cuda.is_available() else "cpu")Y
...
def objective(trial):
    ...
    # Return negative F1-score for maximization
    return -val_f1


# Modified Train Function with Early Stopping
def train_model(train_data, val_data, test_data, task, num_epochs=50, batch_size=8, learning_rate=0.001, l2_lambda=1e-4, hidden_size=128, patience=5):
    ...
    train_tensor = torch.tensor(np.array(train_xyz), dtype=torch.float32).view(len(train_xyz), -1)
    val_tensor = torch.tensor(np.array(val_xyz), dtype=torch.float32).view(len(val_xyz), -1)
    ...
    output_size = 1 if task != 2 else 5  # Binary or Multiclass
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, labels in train_loader:
            ...
            loss = criterion(outputs, labels)
            loss += weight_decay(model, l2_lambda, device)
        ...
        print(f"Epoch [{epoch+1}/{num_epochs}] - Loss: {running_loss:.4f}, Val Loss: {val_loss:.4f}, Val F1: {val_f1:.4f}")
    ...
    return val_f1  # Return validation F1 for Optuna
